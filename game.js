...
const BOSS_LEVELS = {
  easy: {
    label: "Nível 1 · Ameaça Controlada",
    hp: 260,
    damage: 1,
    cooldown: 1350,
    bulletSpeed: 7.5,
  ...
  normal: {
    label: "Nível 2 · Chefe Adaptativo",
    hp: 430,
    damage: 2,
    cooldown: 1050,
    bulletSpeed: 9.2,
...
const BOSS_PATTERN_LABELS = {
  spread: "Rajada em leque",
  ring: "Explosão circular",
  aimed: "Tiros direcionados",
  homing: "Perseguição adaptativa",
  cross: "Cruzamento de arena",
  spiral: "Espiral variável"
};

const BOSS_VOICE_COUNT = 20;
const BOSS_VOICE_COOLDOWN_MS = 4800;
const BOSS_SPAWN = { x: WORLD.width / 2, y: WORLD.height / 2 };
...
const bossLevelLabel = document.querySelector("#bossLevelLabel");
const bossLevelSelect = document.querySelector("#bossLevel");
const scoreTitle = document.querySelector("#scoreTitle");
const bossPanel = document.querySelector("#bossPanel");
const bossName = document.querySelector("#bossName");
const bossLevelText = document.querySelector("#bossLevelText");
const bossHpFill = document.querySelector("#bossHpFill");
const bossHpText = document.querySelector("#bossHpText");
const bossPatternText = document.querySelector("#bossPatternText");
...
const bossImage = loadImage("assets/boss.png");
...
const SFX = {
  ...
  bossAttack: { src: "assets/sfx_ability.wav", volume: 0.68 },
  bossHit: { src: "assets/sfx_hit.wav", volume: 0.7 },
  bossDeath: { src: "assets/sfx_death.wav", volume: 0.78 }
};
...
const bossVoices = Array.from({ length: BOSS_VOICE_COUNT }, (_, index) => {
  const audio = new Audio(`assets/falas${index + 1}.mp3`);
  audio.volume = 0.64;
  audio.preload = "auto";
  return audio;
});
...
let selectedGameMode = "versus";
let selectedBossLevel = "normal";
let roomCode = "";
let playerRef = null;
let playersRef = null;
let bulletsRef = null;
...
modeButtons.forEach((button) => {
  button.addEventListener("click", () => setSelectedMode(button.dataset.mode || "versus"));
});

if (bossLevelSelect) {
  bossLevelSelect.addEventListener("change", () => {
    selectedBossLevel = BOSS_LEVELS[bossLevelSelect.value] ? bossLevelSelect.value : "normal";
  });
}

function setSelectedMode(mode) {
  selectedGameMode = mode === "coop" ? "coop" : "versus";
  ...
  if (bossLevelLabel) {
    bossLevelLabel.classList.toggle("hidden", selectedGameMode !== "coop");
  }
}
...
async function enterRoom() {
  ...
  try {
    ...
    selectedBossLevel = bossLevelSelect && BOSS_LEVELS[bossLevelSelect.value] ? bossLevelSelect.value : selectedBossLevel;

    await ensureMatchExists();

    if (isCoopMode()) {
      await ensureBossExists();
    }
...
async function ensureBossExists() {
  if (!db || !roomCode || !match || !isCoopMode()) return;

  const levelKey = getBossLevelKey();
  const config = getBossLevelConfig();
  const ref = db.ref(`rooms/${roomCode}/boss`);

  const result = await ref.transaction((current) => {
    if (current && current.level === levelKey && Number(current.hp) > 0 && !current.dead) {
      return current;
    }

    return createBossState(levelKey, config);
  });

  boss = result.snapshot.val() || boss;
}
...
function subscribeRoom() {
  ...
  bossRef = db.ref(`rooms/${roomCode}/boss`);
  ...
  matchRef.on("value", (snapshot) => {
    ...
    if (match && match.mode) {
      selectedGameMode = match.mode;
      selectedBossLevel = getBossLevelKey();
    }
...
async function restartRoom() {
  ...
  const mode = isCoopMode() ? "coop" : "versus";
  const bossLevel = getBossLevelKey();
  match = { ...(match || {}), mapIndex: nextMapIndex, mode, bossLevel };

  updates[`rooms/${roomCode}/bullets`] = null;
  updates[`rooms/${roomCode}/powerups`] = null;
  updates[`rooms/${roomCode}/boss`] = mode === "coop" ? createBossState(bossLevel, BOSS_LEVELS[bossLevel] || BOSS_LEVELS.normal) : null;
...
async function endCoopMatch(victory) {
  if (!matchRef || !match || match.status === "ended") return;
  ...
  await matchRef.transaction((current) => {
    ...
    return {
      ...current,
      status: "ended",
      winnerId: victory ? "players" : "boss",
      winnerName: victory ? "Equipe" : "Boss",
      endedAt: Date.now(),
      bossDefeated: Boolean(victory)
    };
  });

  if (victory && bossRef) {
    await bossRef.update({ dead: true, hp: 0, defeatedAt: Date.now() });
  }
}
...
function updateOwnedBullets() {
  ...
  Object.entries(bullets).forEach(([bulletId, bullet]) => {
    const controlsBossBullet = isCoopMode() && bullet.owner === "boss" && isBossController();
    if (bullet.owner !== playerId && !controlsBossBullet) return;
    ...
    if (bullet.homing) {
      const target = bullet.owner === "boss"
        ? findNearestAlivePlayer(bullet.x, bullet.y, 760)
        : (isCoopMode() ? findBossTarget(bullet.x, bullet.y, 820) : findNearestEnemy(bullet.x, bullet.y, 520));
      if (target) {
...
function findNearestAlivePlayer(x, y, maxDistance) {
  let nearest = null;
  let nearestDistance = maxDistance;

  Object.values(players).forEach((player) => {
    if (!player || player.dead || player.hp <= 0) return;
...
function findBossTarget(x, y, maxDistance) {
  if (!boss || boss.dead || boss.hp <= 0) return null;

  const distance = Math.hypot((boss.x || BOSS_SPAWN.x) - x, (boss.y || BOSS_SPAWN.y) - y);
  return distance <= maxDistance ? boss : null;
}
...
function isCoopMode() {
  return Boolean((match && match.mode === "coop") || selectedGameMode === "coop");
}

function getBossLevelKey() {
  if (match && BOSS_LEVELS[match.bossLevel]) return match.bossLevel;
  return BOSS_LEVELS[selectedBossLevel] ? selectedBossLevel : "normal";
}

function getBossLevelConfig() {
  return BOSS_LEVELS[getBossLevelKey()] || BOSS_LEVELS.normal;
}

function createBossState(levelKey, config) {
  return {
    id: "boss",
    name: "Boss Adaptativo",
    level: levelKey,
    x: BOSS_SPAWN.x,
    y: BOSS_SPAWN.y,
    hp: config.hp,
...
function updateBossAI(timestamp) {
  if (!isCoopMode() || !boss || !bossRef || !db || !roomCode || isMatchEnded()) return;
  if (boss.dead || Number(boss.hp || 0) <= 0) return;
  if (!isBossController()) return;
  if (timestamp - lastBossAiAt < 70) return;
  lastBossAiAt = timestamp;

  const alivePlayers = Object.values(players).filter((player) => player && !player.dead && player.hp > 0);
  if (!alivePlayers.length) return;

  const config = BOSS_LEVELS[boss.level] || getBossLevelConfig();
  const currentBoss = sanitizeBossState(boss, config);
  const nearest = findNearestAlivePlayer(currentBoss.x, currentBoss.y, WORLD.width + WORLD.height) || alivePlayers[0];
  const centroid = getPlayersCentroid(alivePlayers);
  const hpPercent = clamp((currentBoss.hp || 0) / (currentBoss.maxHp || config.hp), 0, 1);
  const orbit = (Date.now() / 650) + (currentBoss.attackSeq || 0);
  const targetX = (centroid.x * 0.55) + (nearest.x * 0.45) + Math.cos(orbit) * (160 + (1 - hpPercent) * 120);
  const targetY = (centroid.y * 0.55) + (nearest.y * 0.45) + Math.sin(orbit) * (130 + (1 - hpPercent) * 110);
  const angleToTarget = Math.atan2(targetY - currentBoss.y, targetX - currentBoss.x);
  const distanceToTarget = Math.hypot(targetX - currentBoss.x, targetY - currentBoss.y);
  const step = Math.min(distanceToTarget, config.moveSpeed * (1.1 + (1 - hpPercent) * 0.65));

  const nextBoss = {
    ...currentBoss,
    x: clamp(currentBoss.x + Math.cos(angleToTarget) * step, config.radius, WORLD.width - config.radius),
    y: clamp(currentBoss.y + Math.sin(angleToTarget) * step, config.radius, WORLD.height - config.radius),
    updatedAt: Date.now()
  };

  const now = Date.now();
  const adaptiveCooldown = Math.max(360, config.cooldown - alivePlayers.length * 70 - (1 - hpPercent) * 260);

  if (now - (currentBoss.lastAttackAt || 0) >= adaptiveCooldown) {
    const pattern = chooseBossPattern(nextBoss, alivePlayers, config, hpPercent);
    const attackSeq = (currentBoss.attackSeq || 0) + 1;
    ...
    bossFirePattern(pattern, alivePlayers, config, attackSeq);
  ...
  bossRef.update({
    ...
    attackSeq: boss.attackSeq || 0,
    lastAttackAt: boss.lastAttackAt || 0,
    updatedAt: firebase.database.ServerValue.TIMESTAMP
  }).catch((error) => console.warn("Falha ao sincronizar boss:", error));
}

function chooseBossPattern(currentBoss, alivePlayers, config, hpPercent) {
  const distances = alivePlayers.map((player) => Math.hypot(player.x - currentBoss.x, player.y - currentBoss.y));
  const closest = Math.min(...distances);
  const farthest = Math.max(...distances);
  const spread = farthest - closest;
  const fastPlayers = alivePlayers.filter((player) => {
    const charConfig = CHARACTER_DATA[player.character] || CHARACTER_DATA.blue;
    return charConfig.speed >= 4.8 || hasTimedBuff(player, "speedAmplifyUntil") || hasTimedBuff(player, "wallWalkUntil");
  }).length;

  if (Math.random() < config.chaos) return randomFrom(["spread", "ring", "aimed", "homing", "cross", "spiral"]);
  if (hpPercent <= 0.28) return randomFrom(["spiral", "cross", "homing"]);
  if (closest < 250 || spread < 280) return "ring";
  if (fastPlayers >= Math.max(1, Math.ceil(alivePlayers.length / 2))) return "homing";
  if (alivePlayers.length >= 2 && spread > 620) return "aimed";
  return "spread";
}

function bossFirePattern(pattern, alivePlayers, config, attackSeq) {
  const origin = boss || BOSS_SPAWN;
  const target = findNearestAlivePlayer(origin.x, origin.y, WORLD.width + WORLD.height) || alivePlayers[0];
  const baseAngle = Math.atan2(target.y - origin.y, target.x - origin.x);
  const levelScale = config === BOSS_LEVELS.hard ? 1.25 : config === BOSS_LEVELS.normal ? 1.08 : 1;

  if (pattern === "ring") {
    const count = Math.round(14 * levelScale);
    for (let i = 0; i < count; i++) {
      createBossBullet((Math.PI * 2 / count) * i + attackSeq * 0.08, { speed: config.bulletSpeed * 0.9, radius: 9 });
    }
    return;
  }
  ...
  if (pattern === "aimed") {
    alivePlayers.forEach((player) => {
      ...
      [-0.12, 0, 0.12].forEach((offset) => createBossBullet(angle + offset, { speed: config.bulletSpeed, radius: 8 }));
    });
    return;
  }
  ...
  if (pattern === "homing") {
    alivePlayers.forEach((player) => {
      const angle = Math.atan2(player.y - origin.y, player.x - origin.x);
      createBossBullet(angle, { speed: config.bulletSpeed * 0.82, homing: true, radius: 10, color: "#9b7bff" });
    });
    return;
  }
  ...
  if (pattern === "cross") {
    ...
    angles.forEach((angle) => {
      createBossBullet(angle + attackSeq * 0.04, { speed: config.bulletSpeed * 0.78, radius: 13, throughWalls: true, damage: config.damage + 1, color: "#ff3838" });
    });
    if (config === BOSS_LEVELS.hard) {
      angles.forEach((angle) => createBossBullet(angle + Math.PI / 4, { speed: config.bulletSpeed * 0.7, radius: 10, throughWalls: true, color: "#ff7a18" }));
    }
    return;
  }

  if (pattern === "spiral") {
    const count = config === BOSS_LEVELS.hard ? 12 : 9;
    for (let i = 0; i < count; i++) {
      createBossBullet(baseAngle + attackSeq * 0.38 + i * 0.55, { speed: config.bulletSpeed * (0.72 + i * 0.025), radius: 8, color: "#ffad42" });
    }
    return;
  }

  const offsets = config === BOSS_LEVELS.hard ? [-0.48, -0.32, -0.16, 0, 0.16, 0.32, 0.48] : [-0.34, -0.17, 0, 0.17, 0.34];
  offsets.forEach((offset) => createBossBullet(baseAngle + offset, { speed: config.bulletSpeed, radius: 8 }));
}

function createBossBullet(angle, options = {}) {
  if (!db || !roomCode || !boss || !isCoopMode()) return;

  const config = BOSS_LEVELS[boss.level] || getBossLevelConfig();
  const radius = options.radius || 9;
  const speed = options.speed || config.bulletSpeed;
  ...
  bulletRef.set({
    ...
    ownerName: "Boss",
    boss: true,
    x: boss.x + Math.cos(angle) * (getBossRadius() + 16),
    y: boss.y + Math.sin(angle) * (getBossRadius() + 16),
    vx: Math.cos(angle) * speed,
    vy: Math.sin(angle) * speed,
    damage: options.damage || config.damage,
    radius,
    color: options.color || "#ff3838",
    ricochet: false,
...
function detectBossHits() {
  if (!isCoopMode() || !boss || boss.dead || boss.hp <= 0 || isMatchEnded()) return;

  Object.entries(bullets).forEach(([bulletId, bullet]) => {
    if (!bullet || bullet.owner !== playerId) return;
    if (bullet.hitMap && bullet.hitMap.boss) return;

    let hit = false;
    const radius = getBossRadius();
    ...
    if (bullet.laser) {
      ...
      ) <= radius + (bullet.radius || 12);
    } else {
      hit = Math.hypot((bullet.x || 0) - boss.x, (bullet.y || 0) - boss.y) <= radius + (bullet.radius || BULLET_RADIUS);
    }

    if (hit) {
      applyBossHit(bulletId, bullet);
    }
  });
}

async function applyBossHit(bulletId, bullet) {
  if (!bossRef || !boss || boss.dead || boss.hp <= 0) return;
  ...
  if (bullet.pierce || bullet.laser) {
    await bulletRef.child("hitMap/boss").set(true);
  } else {
    await bulletRef.remove();
  }

  const hpRef = bossRef.child("hp");
  ...
  if (result.committed) {
    await addScore(playerId, damage);
    bossRef.update({ lastHitBy: playerId, lastHitAt: firebase.database.ServerValue.TIMESTAMP }).catch(() => {});

    if (Math.random() < 0.22) playBossVoice();
    playSound("bossHit", 0.44);

    const newHp = Number(result.snapshot.val() || 0);
    if (newHp <= 0) {
      playSound("bossDeath");
      playBossVoice(20, true);
      await endCoopMatch(true);
    }
  }
}

function getPlayersCentroid(list) {
  const sum = list.reduce((acc, player) => {
  ...
  return {
    x: sum.x / Math.max(1, list.length),
    y: sum.y / Math.max(1, list.length)
  };
}

function isBossController() {
  if (!isCoopMode() || !localPlayer) return false;

  const aliveOrPresent = Object.values(players)
  ...
  return Boolean(aliveOrPresent[0] && aliveOrPresent[0].id === playerId);
}

function getBossRadius() {
  const config = getBossLevelConfig();
  return Number((boss && boss.radius) || config.radius || 64);
}

function sanitizeBossState(currentBoss, config) {
  if (!currentBoss) return createBossState(getBossLevelKey(), config);

  const radius = Number(currentBoss.radius || config.radius || 64);
  const maxHp = Math.max(1, Number(currentBoss.maxHp || config.hp));
  return {
    ...currentBoss,
    level: BOSS_LEVELS[currentBoss.level] ? currentBoss.level : getBossLevelKey(),
    x: Number.isFinite(Number(currentBoss.x)) ? clamp(Number(currentBoss.x), radius, WORLD.width - radius) : BOSS_SPAWN.x,
    y: Number.isFinite(Number(currentBoss.y)) ? clamp(Number(currentBoss.y), radius, WORLD.height - radius) : BOSS_SPAWN.y,
    hp: Math.max(0, Math.min(maxHp, Number.isFinite(Number(currentBoss.hp)) ? Number(currentBoss.hp) : maxHp)),
    maxHp,
    radius,
    pattern: currentBoss.pattern || "spread",
    attackSeq: Number.isFinite(Number(currentBoss.attackSeq)) ? Number(currentBoss.attackSeq) : 0,
    dead: Boolean(currentBoss.dead || Number(currentBoss.hp || maxHp) <= 0)
  };
}

function playBossVoice(index, force = false) {
  const now = Date.now();
  if (!force && now - lastBossVoiceAt < BOSS_VOICE_COOLDOWN_MS) return;
  lastBossVoiceAt = now;

  const safeIndex = Number.isInteger(index) ? clamp(index, 1, BOSS_VOICE_COUNT) : Math.floor(randomBetween(1, BOSS_VOICE_COUNT + 1));
  const audio = bossVoices[safeIndex - 1];
  if (!audio) return;

  try {
    const clone = audio.cloneNode();
    clone.volume = audio.volume || 0.64;
    clone.play().catch(() => {});
  } catch (error) {
...
function detectHits() {
  ...
  Object.entries(bullets).forEach(([bulletId, bullet]) => {
    if (isCoopMode() && bullet.owner !== "boss") return;
    if (!isCoopMode() && bullet.owner === "boss") return;
...
async function addScore(attackerId, amount = 1) {
  if (!attackerId || !players[attackerId]) return;
...
function drawBoss() {
  if (!isCoopMode() || !boss || boss.dead || boss.hp <= 0) return;

  const radius = getBossRadius();
  const x = Number(boss.x || BOSS_SPAWN.x) - camera.x;
  const y = Number(boss.y || BOSS_SPAWN.y) - camera.y;

  if (x < -radius - 120 || x > canvas.width + radius + 120 || y < -radius - 120 || y > canvas.height + radius + 120) return;

  const hpPercent = clamp((boss.hp || 0) / (boss.maxHp || 1), 0, 1);
  const pulse = 1 + Math.sin(Date.now() / 170) * 0.035;
  const size = radius * 2.45 * pulse;

  ctx.save();
  ctx.shadowColor = "rgba(255, 56, 56, 0.68)";
  ctx.shadowBlur = 28;

  ctx.beginPath();
  ctx.arc(x, y, radius + 18 + Math.sin(Date.now() / 120) * 3, 0, Math.PI * 2);
  ctx.strokeStyle = "rgba(255, 56, 56, 0.45)";
  ctx.lineWidth = 5;
  ctx.stroke();

  if (isImageReady(bossImage)) {
    ctx.drawImage(bossImage, x - size / 2, y - size / 2, size, size);
  } else {
  ...
  ctx.textAlign = "center";
  ctx.fillStyle = "#ffffff";
  ctx.fillText(boss.name || "Boss Adaptativo", x, y - radius - 34);

  const barWidth = Math.max(116, radius * 2.2);
  const barHeight = 10;
  ctx.fillStyle = "rgba(255,255,255,0.16)";
  ctx.fillRect(x - barWidth / 2, y + radius + 22, barWidth, barHeight);
  ctx.fillStyle = hpPercent > 0.52 ? "#ffad42" : "#ff3838";
  ctx.fillRect(x - barWidth / 2, y + radius + 22, barWidth * hpPercent, barHeight);

  ctx.font = "800 12px Arial";
  ctx.fillStyle = "rgba(255,255,255,0.84)";
  ctx.fillText(BOSS_PATTERN_LABELS[boss.pattern] || "Padrão adaptativo", x, y + radius + 48);
  ctx.restore();
}
...
function drawMiniMap() {
  ...
  if (isCoopMode() && boss && !boss.dead && boss.hp > 0) {
    ctx.fillStyle = "#ff3838";
    ctx.beginPath();
    ctx.arc(
      x + ((boss.x || BOSS_SPAWN.x) / WORLD.width) * w,
      y + ((boss.y || BOSS_SPAWN.y) / WORLD.height) * h,
      6,
      0,
      Math.PI * 2
    );
...
function renderScoreboard() {
  ...
  if (scoreTitle) {
    scoreTitle.textContent = isCoopMode() ? "Dano no boss" : "Pontuação";
  }
  ...
  scoreboard.innerHTML = list.map((player) => {
    ...
    const me = player.id === playerId ? "Você" : (isCoopMode() ? "Aliado" : "Rival");

    return `
      <div class="player-row">
        <div class="player-name">
          <span>${escapeHtml(player.name || "Jogador")} <small>(${me})</small></span>
          <span>${score} ${isCoopMode() ? "dano" : "pts"}</span>
        </div>
...
function updateStatus() {
  ...
  if (isCoopMode()) {
    const bossHp = boss ? `${Math.max(0, Math.ceil(boss.hp || 0))}/${Math.ceil(boss.maxHp || 0)}` : "carregando";
    statusBar.textContent = `Modo cooperativo. Aliados online: ${count}. Boss: ${bossHp}. Padrão: ${BOSS_PATTERN_LABELS[boss && boss.pattern] || "adaptativo"}.`;
    return;
  }
...
function showEndOverlay() {
  ...
  if (isCoopMode()) {
    const topDamage = sorted[0] ? sorted[0].score || 0 : 0;
    winnerText.textContent = match.bossDefeated ? "Equipe venceu" : "Boss venceu";
    winnerDetail.textContent = match.bossDefeated
      ? `O boss foi derrotado. Maior dano causado: ${topDamage}.`
      : "O tempo acabou e o boss continuou de pé.";
...
function renderBossPanel() {
  ...
  if (!isCoopMode() || !boss || !bossPanel) return;

  const config = BOSS_LEVELS[boss.level] || getBossLevelConfig();
  const hp = Math.max(0, Math.ceil(Number(boss.hp || 0)));
  const maxHp = Math.max(1, Math.ceil(Number(boss.maxHp || config.hp)));
  const percent = clamp((hp / maxHp) * 100, 0, 100);

  if (bossName) bossName.textContent = boss.name || "Boss Adaptativo";
  if (bossLevelText) bossLevelText.textContent = config.label;
  if (bossHpFill) bossHpFill.style.width = `${percent}%`;
  if (bossHpText) bossHpText.textContent = `Vida: ${hp}/${maxHp}`;
  if (bossPatternText) bossPatternText.textContent = `Padrão: ${BOSS_PATTERN_LABELS[boss.pattern] || "adaptativo"}`;
}

function updateModePanels() {
  if (bossPanel) {
    bossPanel.classList.toggle("hidden", !isCoopMode());
  }
